...
Control tables
- Age x Sex
- Household size x Household type
- Marital status 
...
ggplot() +
  geom_sf(data = cp, aes(color = "puma", fill = NULL)) +
  geom_sf(data = ct, aes(color = "tract", fill = NULL))
```
...
pums <- get_pums(
  ...
  year = 2018,
  recode = TRUE, 
  key = "ef0bd4a8f47666930ef383cbea8899dc2ee28fd3"
) %>%
...
```{r}
replicates <- map(fits[5], ~ ml_replicate(.x, algorithm = "trs"))
ml_replicate(fits)
```
